...
public class ProgressButton extends CompoundButton {
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		...
		final TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ProgressButton, defStyle,
				R.style.ProgressButton_Pin_Compat);
		final Resources res = getResources();

		mProgress = a
				.getInteger(R.styleable.ProgressButton_progress, mProgress);
		...
		int shadowDrawable = a.getResourceId(
				R.styleable.ProgressButton_shadowDrawable1,
		...
		setBackgroundDrawable(a
				.getDrawable(R.styleable.ProgressButton_android_background));

		mAnimating = a.getBoolean(R.styleable.ProgressButton_animating,
				mAnimating);
		mAnimationSpeed = a.getInteger(
				R.styleable.ProgressButton_animationSpeed, mAnimationSpeed);
		mAnimationDelay = a.getInteger(
				R.styleable.ProgressButton_animationDelay, mAnimationDelay);
		mAnimationStripWidth = a.getInteger(
				R.styleable.ProgressButton_animationStripWidth,
	...
	public void setProgressAndMax(int progress, int max) {
		if (progress > max || progress < 0) {
			throw new IllegalArgumentException(
					String.format("Progress (%d) must be between %d and %d",
							progress, 0, max));
		} else if (max <= 0) {
			throw new IllegalArgumentException(String.format(
					"Max (%d) must be > 0", max));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mAnimating) {
			canvas.drawArc(mTempRectF, -90 + (360 * mAnimationProgress / mMax),
					mAnimationStripWidth, true, mProgressPaint);
		}
