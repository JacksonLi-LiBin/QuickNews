...
public class TupianSinaQuTuFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
	...
	private Handler handler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 0:
				getMyActivity().setCacheStr("TupianSinaQuTuFragment" + currentPage, result);
				if (isRefresh) {
					isRefresh = false;
	...
	private void loadData(String url) {
		if (getMyActivity().hasNetWork()) {
			...
			String result = getMyActivity().getCacheStr("TupianSinaQuTuFragment" + currentPage);
