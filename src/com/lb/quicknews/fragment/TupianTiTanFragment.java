...
public class TupianTiTanFragment extends BaseFragment
	...
	private Handler handler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 0:
				getMyActivity().setCacheStr("TupianTiTanFragment", result);
				if (isRefresh) {
					isRefresh = false;
					photoAdapter.clear();
					listModles.clear();
	...
	void init() {
		...
		if (count != -1) {
			if ((lastupdatetimetitian + (24 * 60 * 60 * 1000) < System.currentTimeMillis())) {
				...
				PreferencesUtils.putLong(getActivity(), "lastupdatetimetitian", lastupdatetimetitian);
	...
	private void loadData(String url) {
		if (getMyActivity().hasNetWork()) {
			...
			String result = getMyActivity().getCacheStr("TupianTiTanFragment");
