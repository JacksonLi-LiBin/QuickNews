...
public class VideoYuLeFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
	...
	private Handler handler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 0:
				getMyActivity().setCacheStr("VideoYuLeFragment" + currentPage, result);
				if (isRefresh) {
					isRefresh = false;
					videoAdapter.clear();
					listModles.clear();
	...
	private void loadData(String url) {
		if (getMyActivity().hasNetWork()) {
			...
			String result = getMyActivity().getCacheStr("VideoYuLeFragment" + currentPage);
