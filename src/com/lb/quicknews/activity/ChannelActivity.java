...
public class ChannelActivity extends BaseActivity implements
	...
	// 是否在移动，由于这边是动画介绍后才进行的数据更替，设置这个限制为了避免操作太多太频繁造成的数据错乱
	...
	void initData() {
		...
		otherChannelList = ((ArrayList<ChannelItem>) ChannelManage.getManage(
				App.getApplication().getSQLHelper()).getOtherChannel());
		setData();
	}
						...
						public void run() {
							try {
								...
								ChannelManage.getManage(
										App.getApplication().getSQLHelper())
	...
	void saveChannel() {
		ChannelManage.getManage(App.getApplication().getSQLHelper())
				.deleteAllChannel();
		ChannelManage.getManage(App.getApplication().getSQLHelper())
